			...
			<div class="content">
				<div class="col-xs-10 col-sm-10 col-md-10 col-lg-10" style="">
					...
					<div class="card card-secondary" style="margin-left: 30px;">
						...
						<!-- form start -->
						<form class="form-inline" action="${base }/admin_user_list"
							method="get">
								...
								<div class="d-flex flex-row ml-1">
									...
									<div class="" style="">
										<a class=" btn btn-block btn-secondary ml-1"
											href="${base }/admin_user" role="button">Add New</a>
									</div>
								</div>
							</div>
								...
								<tbody>
									<c:forEach items="${userWithPaging.data}" var="user"
										...
										<tr>
											...
											<td><c:forEach items="${roles}" var="roles">
													<c:forEach items="${user.roles}" var="uroles">
														<c:if test="${roles} == ${user.roles}"> roles.name</c:if>
													</c:forEach>
												</c:forEach></td>

											<td><span style="font-size: 15px"
												id="status_${user.id }"> <c:choose>
												...
												<button class="btn bg-gradient-success mr-1"
													...
													type="button"
													onclick="activeStatusPro('${base }', ${user.id })">on</button>

												<button class="btn btn-danger mr-1"
													style="padding: 3px 6px 3px 6px; font-size: 12px"
													type="button"
													onclick="inActiveStatusPro('${base }', ${user.id })">off</button></td>

											<!-- Edit -->
											<td>
												<div>
													<a class="btn bg-gradient-success"
														href="${base }/admin_user_${user.id}" role="button"><i
														class="fas fa-edit"></i></a>
													<!-- Delete -->
													<button class="btn btn-danger" role="button" type="button"
														onclick="deleteUser('${base }', ${user.id })">
														<i class="far fa-trash-alt"></i>
													</button>
													<span id="statusDelete_${user.id }"></span>
												</div>
											</td>
										</tr>
									</c:forEach>

...
<body class="hold-transition sidebar-mini">
			...
			<div class="content">
				<div class="col-xs-10 col-sm-10 col-md-10 col-lg-10" style="">
					...
					<div class="card card-secondary" style="margin-left: 30px;">
						...
						<!-- form start -->
						<form class="form-inline" action="${base }/admin_role_list"
							method="get">
								...
								<div class="d-flex flex-row ml-1">
									...
									<div class="" style="">
										<a class=" btn btn-block btn-secondary ml-1"
											href="${base }/admin_role" role="button">Add New</a>
									</div>
								</div>
							</div>
							<table class="table table-striped">
								...
								<tbody>
									<c:forEach items="${rolesWithPaging.data}" var="role"
										varStatus="loop">
										<tr>
											<th scope="row">${loop.index + 1}</th>

											<td><i class="fas fa-barcode mr-2"></i>${role.name }</td>

											<td>
												<div style="width: 300px">${role.description}</div>
											</td>

											<td><span style="font-size: 15px"
												id="status_${role.id }"> <c:choose>
												...
												<button class="btn bg-gradient-success mr-1"
													...
													type="button"
													onclick="activeStatusSto('${base }', ${role.id })">on</button>

												<button class="btn btn-danger mr-1"
													style="padding: 3px 6px 3px 6px; font-size: 12px"
													type="button"
													onclick="inActiveStatusSto('${base }', ${role.id })">off</button></td>
											...
											<td>
												<div>
													...
													<a class="btn bg-gradient-success"
														href="${base }/admin_role_${role.id}" role="button"><i
														class="fas fa-edit"></i></a>
													<!-- Delete -->
													<button class="btn btn-danger" role="button" type="button"
														onclick="deleteRo('${base }', ${role.id })">
														<i class="far fa-trash-alt"></i>
													</button>
													<span id="statusDelete_${role.id }"> </span>
												</div>
											</td>
										</tr>
									</c:forEach>
	...
	<script type="text/javascript">
			$( document ).ready(function() {
				...
				$("#paging").pagination({
					currentPage: ${rolesWithPaging.currentPage},
			        items: ${rolesWithPaging.totalItems},
			        itemsOnPage: 5,
			        cssStyle: 'dark-theme',

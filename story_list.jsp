...
<body class="hold-transition sidebar-mini">
			...
			<div class="content">
				<div class="col-xs-10 col-sm-10 col-md-10 col-lg-10" style="">
					...
					<div class="card card-secondary" style="margin-left: 30px;">
						...
						<!-- form start -->
						<form class="form-inline" action="${base }/admin_story_list"
							method="get">
							...
							<table class="table table-striped">
								...
								<tbody>
									<c:forEach items="${storiesWithPaging.data}" var="story"
										varStatus="loop">
										<tr>
											<th scope="row">${loop.index + 1}</th>

											<td><img src="${base }/upload/${story.avatar}"
												width="75" height="80"></td>

											<td><i class="fas fa-barcode mr-2"></i>${story.title }</td>

											<td>
												<div style="width: 300px">${story.shortDes}</div>
											</td>

											<td><span style="font-size: 15px"
												id="status_${story.id }"> <c:choose>
														<c:when test="${story.status }">
												...
												<button class="btn bg-gradient-success mr-1"
													...
													type="button"
													onclick="activeStatusSto('${base }', ${story.id })">on</button>

												<button class="btn btn-danger mr-1"
													style="padding: 3px 6px 3px 6px; font-size: 12px"
													type="button"
													onclick="inActiveStatusSto('${base }', ${story.id })">off</button></td>

											<!-- Edit -->
											<td>
												<div>
													<a class="btn bg-gradient-success"
														href="${base }/admin_story_${story.id}" role="button"><i
														class="fas fa-edit"></i></a>
													<!-- Delete -->
													<button class="btn btn-danger" role="button" type="button"
														onclick="deleteSto('${base }', ${story.id })">
														<i class="far fa-trash-alt"></i>
													</button>
													<span id="statusDelete_${story.id }"> </span>
												</div>
											</td>
										</tr>
									</c:forEach>
	...
	<script type="text/javascript">
			$( document ).ready(function() {
				...
				$("#paging").pagination({
					currentPage: ${storiesWithPaging.currentPage},
			        items: ${storiesWithPaging.totalItems},
			        itemsOnPage: 5,
			        cssStyle: 'dark-theme',
